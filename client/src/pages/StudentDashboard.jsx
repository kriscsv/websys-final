...
// ── API hooks (swap BASE_URL for your real backend later) ─────────────────────
...
function useCurrentUser() {
  ...
  useEffect(() => {
    // TODO: GET /api/auth/me  — returns the logged-in student's profile
    // Uncomment and remove the stub below when backend is ready:
    //
    ...
    // ── STUB (remove when DB is ready) ──
    setUser(null);   // null = no data yet
    setLoading(false);
  }, []);
...
function useMyDocuments() {
  ...
  const fetch_ = useCallback(() => {
    setLoading(true);
    // TODO: GET /api/documents/mine  — returns documents uploaded by this student
    //
    // fetch(`${BASE_URL}/documents/mine`, { credentials: "include" })
    //   .then(r => r.json())
...
function useLibrary() {
  ...
  useEffect(() => {
    // TODO: GET /api/documents  — returns all approved MFO docs (filterable by college, category, etc.)
    //
...
async function uploadDocument(formData) {
  // TODO: POST /api/documents  — multipart/form-data with file, title, category
  //
  // const res = await fetch(`${BASE_URL}/documents`, {
  //   method: "POST",
  //   credentials: "include",
  //   body: formData,   // FormData object (file + fields)
  // });
  ...
  return (
    <section style={{ position:"relative", zIndex:10, padding:"0 80px 0" }}>
      ...
      <p style={{ color:"rgba(255,255,255,0.45)", fontSize:"13px", margin:"0 0 28px", lineHeight:1.7 }}>
        {user
          ? `${user.college} · ${user.course} · ${user.year} · ${user.bloc}`
  ...
  return (
    <div
      ...
      style={{
        ...fade.style,
        display:"grid",
        gridTemplateColumns: showUploader
          ? "2fr 1fr 1fr 110px 70px 70px"
          : "2fr 1fr 110px 70px 80px 60px",
    ...
    >
      <div style={{ minWidth:0 }}>
        <p style={{ color:"#111", fontWeight:500, fontSize:"13px", margin:"0 0 3px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
          {doc.title}
        </p>
      ...
      {showUploader && (
        <span style={{ color:"#666", fontSize:"12px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
          {doc.uploaderName}
        </span>
      )}
      ...
      {!showUploader ? (
        <span style={{
          background: sts.bg, color: sts.text,
    ...
    >
      <div style={{
        ...
        <div
          ...
          style={{
            border: dragging ? "2px dashed #2d3a8c" : file ? "2px dashed #22c55e" : "2px dashed #e0e0e0",
          ...
          {file ? (
            ...
            <>
              ...
              <p style={{ color:"#aaa", fontSize:"13px", margin:0 }}>
                Drag & drop or <span style={{ color:"#2d3a8c", fontWeight:600 }}>browse</span>
              </p>
        ...
        <div style={{ marginBottom:"12px" }}>
          ...
          <input
            type="text" placeholder="e.g. Smart Irrigation System Using IoT"
        ...
        <div style={{ display:"flex", gap:"10px" }}>
          <button
            ...
            style={{
              ...
              color:"#666", fontSize:"13px", fontWeight:600, cursor:"pointer",
            }}
          >
            Cancel
          </button>
          <button
            ...
            style={{
              ...
              background: canSubmit ? "#e86c1a" : "#f0a070",
...
// ── Profile slide-over (sign out lives here) ──────────────────────────────────
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ background:"#2d3a8c", padding:"28px 28px 24px", position:"relative", flexShrink:0 }}>
          ...
          <h2 style={{ color:"#fff", fontWeight:700, fontSize:"16px", margin:"0 0 4px" }}>
            {user ? user.fullName : "—"}
          </h2>
        ...
        <div style={{ padding:"22px 28px", borderBottom:"1px solid #f0f0f0", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:"8px", marginBottom:"14px" }}>
            {[
              { val: myDocs.length, label:"Total",    color:"#2d3a8c" },
              { val: approved,      label:"Approved", color:"#166534" },
              { val: pending,       label:"Pending",  color:"#854f0b" },
            ].map(s => (
              <div key={s.label} style={{
        ...
        <div style={{ padding:"22px 28px", flex:1 }}>
          ...
          {myDocs.length === 0 ? (
            <p style={{ color:"#ccc", fontSize:"13px", textAlign:"center", padding:"20px 0" }}>
              No documents yet.
            </p>
          ) : (
              ...
              return (
                <div key={doc.id} style={{
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ color:"#222", fontSize:"12px", fontWeight:500, margin:"0 0 2px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                      {doc.title}
                    </p>
        ...
        {/* ── Sign out — lives here, not in navbar ── */}
...
function DashboardContent({ activeTab, onUpload, myDocs, myLoading, library, libLoading }) {
  ...
  const colHeaders = isMyDocs
    ? ["Document", "Category", "Date", "Size", "Status", ""]
    : ["Document", "Uploaded by", "Category", "Date", "Size", ""];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display:"flex", gap:"10px", alignItems:"center" }}>
          <div style={{
            ...
            background: searchFocus ? "#fff" : "#f4f4f4",
            border: searchFocus ? "1.5px solid #2d3a8c" : "1.5px solid #f4f4f4",
      ...
      <div style={{ padding:`18px ${H_PAD} 0` }}>
        ...
        <div style={{ paddingBottom:"48px" }}>
          {loading ? (
            ...
            filtered.map((doc, i) => (
              <DocRow key={doc.id} doc={doc} showUploader={!isMyDocs} index={i} />
            ))
          )}
        </div>
      </div>
      ...
      <div style={{ padding:`18px ${H_PAD}`, display:"flex", alignItems:"center", gap:"10px" }}>
        ...
        <p style={{ color:"#bbb", fontSize:"12px", margin:0 }}>
          Students can upload documents. Editing and deletion are managed by PKMD staff only.
        </p>
      </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", backgroundColor:"#2d3a8c", minHeight:"100vh" }}>
      ...
      <DashboardContent
        ...
        myDocs={myDocs}   myLoading={myLoading}
